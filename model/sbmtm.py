...
class sbmtm():
    ...
    def make_graph(self,list_texts, documents = None, counts=True, n_min = None):
        ...
        ## add all documents and words as nodes BIPARTITE GRAPH HERE
    ...
    def print_topics(self,l=0,format='csv',path_save = ''):
        ...
        ## cluster-cluster mixture

        mutual_info = self.pmi_td_tw(l=l)

        df = pd.DataFrame(columns=list_columns, index = range(len(mutual_info[0])))

        for i, col in enumerate(mutual_info):
            df.iloc[:,i] = col
        fname_save = 'topsbm_level_%s_pmi.csv'%(l)
        filename = os.path.join(path_save,fname_save)
        df.to_csv(filename,index=False,na_rep='')
    ...
    def plot_topic_dist(self, l):
        ...
        plt.title(r'Word group membership $P(w | twmodel.print_topic_saliency(path_save=path_data))$')
        plt.xlabel('Topic, tw')
    ...
    ADDED FUNCTIONS
    '''
    def print_hierarchy_json(self):
        hierarchy_dict = self.create_hierarchy_dict(l = self.L)

        with open("topic_hierarchy.json", "w") as outfile:
            json.dump(hierarchy_dict, outfile)

    def create_hierarchy_dict(self, l):
        # recursive
        dict_topics = self.topics(l=l,n=-1)
        topic_hierarchy = {}
        ...
        if l == 0:
            ## Create lowest level of dictionary with name as first word in topic and value as number of words in topic
            topics_formatted = []
            for val in dict_topics.values():
                topics_formatted.append({"name": val[0][0], "value":len(val)})
            return topics_formatted
        # For intermediary levels of hierarchy
        elif l != self.L:
            topic_children = self.create_hierarchy_dict(l = l-1)  # get lower level topics
            topics_formatted = []                            # initailize final topics array
            # iterate through topics at level l in hierarchy and determine its children
            for val in dict_topics.values():
                children = []
                a = [words[0] for words in val]
                # iterate through lower level hierarchy topics and assign children to parent topics
                for tc in topic_children:
                    if tc["name"] in a:
                        children.append(tc)
                topics_formatted.append({"name": val[0][0], "children": children})
            return topics_formatted
        else:
            topic_hierarchy = {"name": dict_topics[0][0][0],
                               "children": self.create_hierarchy_dict(l=l-1)
                              }
    ...
    # Computing saliency
    def compute_saliency(self, l=0):
        """
        Compute saliency value of each word at level l,
        return dictionary sorted by saliency
        """
        dict_groups = self.get_groups(l=l)
        ...
        p_tw_w = dict_groups['p_tw_w']
        # P( T )
        p_tw = np.sum(p_tw_w, axis=1)
        

        # compute K-L divergence
        w_divergence = [self.compute_KL_divergence(p_tw, p_tw_w[:, i]) for i in range(p_tw_w.shape[1])]

        # get P( w )
        p_w = dict_groups['p_w']

        # compute saliency
        saliency = p_w * w_divergence
        words = self.words
        
        saliency_dict = dict(zip(words, saliency))

        # sort words by saliency
        saliency_dict = {k: v for k, v in sorted(saliency_dict.items(), key=lambda item: item[1], reverse=True)}

        return saliency_dict

    def compute_topic_saliency(self, l=0):
        """
        Compute average topic saliency using word saliency values at level l
        return dictionary ordered by saliency
        """
        dict_topics = self.topics(l=l,n=-1)
        dict_saliency = self.compute_saliency(l=l)

        topic_saliency = {}
        for t_num, t_words in dict_topics.items():
            w_saliency = 0
            for tw in t_words:
                for word, saliency in dict_saliency.items():
                    if tw[0] == word:
                        w_saliency += saliency
            avg_t_saliency = w_saliency / len(t_words)
            topic_saliency[t_num] = avg_t_saliency
        
        topic_saliency = {k: v for k, v in sorted(topic_saliency.items(), key=lambda item: item[1])}
        
        return topic_saliency
                


    # helper function for computing saliency
    def compute_KL_divergence(self, p_tw, p_tw_w):
        """
        Compute KL divergence of a word w in relation to a word topic tw
        """
        divergence = 0
        for i in range(len(p_tw_w)):
            p = p_tw_w[i]
            q = p_tw[i]
            assert p >= 0
            assert q >= 0
    ...
    # # Computing similarity: TO DO later
    # def compute_similarity(self):
    #     similarity = []
    #     return similarity

    # # Computing seriation
    # def compute_seriation(self):
    #     seriation = []
    #     return seriation


    # Print saliency, similarity, seriation
    def print_topic_saliency(self, l=0, path_save=''):
        """
        Print word topic information and saliency values at level l to json
        """
        dict_topics = self.topics(l=l,n=-1)
        dict_saliency = self.compute_topic_saliency(l=l)

        topic_sal_ordering = []
        for t_num, t_saliency in dict_saliency.items():
            t_words = []
            t_frequency = []
            for topic, words in dict_topics.items():
                if topic == t_num:
                    for w in words:
                        t_words.append(w[0])
                        t_frequency.append(w[1])
                    break
            topic_sal_ordering.append({"topic_id": t_num, "words": t_words, "frequency": t_frequency, "saliency": t_saliency})

        with open(os.path.join(path_save,"topic_saliency.json"), "w") as outfile:
            json.dump(topic_sal_ordering, outfile)