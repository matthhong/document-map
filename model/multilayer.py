...
class sbmmultilayer:
    ...
    def make_graph(self, list_texts, list_titles=None, list_codes=[], list_coded_words=[]):
        """
        Load a corpus and generate the multilayered network where one layer
        is the multigraph word-document bipartite network and another is the word-code network.
        ...
        list_codes : array of arrays
            relationships between codes and categories
        list_coded_words : array of arrays
            relationships between codes and words
        ...
        # Documents nodes (0), word nodes (1), code nodes (2)
        kind = g.vp["kind"] = g.new_vp("int")
        # Specify Vertex Layers: word node: [0, 1]; doc node: [0]; code node: [1]
        ...
        code_vertices = defaultdict(lambda: g.add_vertex())
        ...
        kind_count = 1
        seen_categories = []
        for i, pair in enumerate(list_codes):
            # Retrieve source and target nodes and add hyperlink edge
            s = code_vertices[pair[0]]
            t = code_vertices[pair[1]]
            name[s] = pair[0]
            name[t] = pair[1]
            
            # if not (pair[0] in seen_categories):
            #     kind_count += 1
            #     seen_categories.append(pair[0])
            # kind[s] = kind_count
            # kind[t] = kind[s]
            kind[s] = 2
            kind[t] = 2
        ...
        # Codes and words
        all_coded_words = []
        for pair in list_coded_words:
            if pair[0] in code_vertices:
                s = code_vertices[pair[0]]
                coded_words = pair[1].split(',')

                if len(coded_words) > 1:
                    for word in coded_words:
                        if word != '' and word in word_vertices:
                            all_coded_words.append(word)

                            t = word_vertices[word]
                            e = g.add_edge(s, t)
                            edgeCount[e] = total_counter[word]
                            edgeType[e] = 1 # Indicates the edge is hyperlink

        kind_count += 1
        s = code_vertices['uncoded']
        name[s] = 'uncoded'
        kind[s] = kind_count
        vlayers[s] = [1]

        for word in word_vertices:
            if not (word in all_coded_words):
                t = word_vertices[word]
                e = g.add_edge(s, t)
                edgeCount[e] = total_counter[word]
                edgeType[e] = 1 # Indicates the edge is hyperlink
    ...
    def fit(self):
        ...
        # Simulated annealing
        # print("equilibrate")
        # gt.mcmc_equilibrate(state, force_niter=40, mcmc_args=dict(beta=np.inf),history=True)  
        ...
        if n_levels == 2:
            # Bipartite network
            self.groups = { 0: self.get_groups(l=0) }
            self.n_levels = len(self.groups)
        # Omit trivial levels: l=L-1 (single group), l=L-2 (bipartite)
        else:
            self.groups = { level: self.get_groups(l=level) for level in range(n_levels - 2) }
            self.n_levels = len(self.groups)
    ...
    def get_groups(self, l=0):
        ...
        # Per-topic document distribution, shape D x B_d
        ...
        result['p_d_tw'] = p_d_tw # Topic proportions over documents
...
# counts = 'edgeCount' in model.g.ep.keys()

# for e in g.edges():
#     if g.ep.edgeType[e] == 0:
#         z1,z2 = state_l_edges[e]
#         if counts:
#             print(g.ep["edgeCount"][e])
#             n_td_tw[z1 , z2] += g.ep["edgeCount"][e]
#         else:
#             n_td_tw[z1, z2] += 1
